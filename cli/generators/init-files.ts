...
import { buildShadcnRendererInfrastructurePlan, rendererMapImportSpecifier } from "./renderer-infrastructure";
...
export type PlannedFile = {
  ...
  contents: string;
  status: FileStatus;
  /** Original UTF-8 contents captured while building the plan. */
  before?: string;
};
...
export function generateConfigContents(ui: {
  adapter: TrimUIAdapterValue;
  shell?: TrimShellValue;
  renderersImport?: string;
}): string {
  const uiBlock =
    ui.adapter === "headless"
      ? `  ui: {\n    adapter: "headless",\n  },\n`
      : `  ui: {\n    adapter: ${JSON.stringify(ui.adapter)},\n    shell: ${JSON.stringify(ui.shell ?? "popover")},\n${ui.renderersImport ? "    renderers: trimRenderers,\n" : ""}  },\n`;
  const rendererImport = ui.renderersImport ? `import { trimRenderers } from "${ui.renderersImport}";\n` : "";
...
export async function buildInitPlan(
  ...
  const files: PlannedFile[] = [
    planFile(
      ...
      generateConfigContents({
        ...
        renderersImport: effectiveAdapter === "shadcn" ? rendererMapImportSpecifier(project.moduleResolution) : undefined,
      }),
  ...
  if (effectiveAdapter === "shadcn") {
    const rendererPlan = await buildShadcnRendererInfrastructurePlan(project.cwd, project.moduleResolution);
    for (const rendererFile of rendererPlan.files) {
      files.push({ path: rendererFile.path, contents: rendererFile.contents, status: rendererFile.status });
    }
  }
  ...
  const plannedFiles = await Promise.all(files.map(async (file) => ({
    ...file,
    before: await readIfExists(project.cwd, file.path),
  })));
  return {
    files: plannedFiles,
    notes,
...
export async function applyInitPlan(
  ...
  const plannedFiles = plan.files.filter((file) => file.status !== "conflict");
  const files = plannedFiles.filter((file) => file.status === "create" || file.status === "update");
  const before = new Map<string, Buffer | undefined>();
  for (const file of plannedFiles) {
    try {
      const stat = await lstat(path.join(cwd, file.path));
      if (!stat.isFile()) throw new UsageError(`${file.path} must be a regular file — nothing was written.`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
    const current = await readIfExists(cwd, file.path);
    const plannedBefore = file.before;
    if (current !== plannedBefore) throw new UsageError(`${file.path} changed during init planning — nothing was written. Retry init.`);
    before.set(file.path, current === undefined ? undefined : Buffer.from(current));
  }
  const createdDirectories: string[] = [];
  const touched: typeof files[number][] = [];
  ...
  try {
    ...
    for (const file of touched.reverse()) {
      ...
      try {
        ...
        if (original === undefined) await unlink(fullPath);
        else await writeFile(fullPath, original);
      } catch { failures.push(file.path); }
    }
    for (const directory of createdDirectories.reverse()) {
      try { await rmdir(directory); } catch { failures.push(directory); }
    }
    if (failures.length) throw new UsageError(`Init failed (${String(error)}); rollback could not restore: ${failures.join(", ")}.`);
    throw error;
  }
}
