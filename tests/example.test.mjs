// Acceptance test for examples/default/ — the canonical, hand-written 0.2
// example. Unlike every other tests/*.mjs (which compile a slice of
// src/**), this test treats the example as an EXTERNAL CONSUMER would: it
...
const exampleDir = path.join(root, 'examples/default');
...
try {
  execFileSync('node', [
    ...
    '--rootDir', 'examples/default', '--outDir', path.relative(root, outDir),
  ], { cwd: root });
